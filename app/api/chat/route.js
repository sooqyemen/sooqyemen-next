...
const CATEGORIES = [
  {
    ...
    name: 'سيارات',
    keywords: [
      'سيارة','سيارات','سياره','مركبة','مركبه','للبيع سيارة','car','cars','kars','carz','كارس','كارز',
      'تويوتا','toyota','كورولا','camry','هوندا','honda','كيا','kia','هيونداي','hyundai','نيسان','nissan'
    ],
  },
  {
    slug: 'realestate',
    name: 'عقارات',
    keywords: [
      'عقار','عقارات','شقة','شقه','شقق','بيت','بيوت','منزل','منازل','فيلا','فلل','عمارة','عماره','محل','أرض','ارض','مزرعة',
      'إيجار','ايجار','للإيجار','للبيع','real estate','realestate','estate','property'
    ],
  },
  {
    slug: 'phones',
    name: 'جوالات',
    keywords: [
      'جوال','جوالات','موبايل','هاتف','هواتف','ايفون','آيفون','iphone','سامسونج','samsung','شاومي','xiaomi','ريدمي','redmi',
      'phone','phones','mobile','mobiles'
    ],
  },
  {
    slug: 'electronics',
    name: 'إلكترونيات',
    keywords: [
      'الكترونيات','إلكترونيات','الكتروني','electronics','كمبيوتر','حاسوب','pc','لابتوب','لاب توب','laptop','macbook',
      'شاشة','monitor','تلفزيون','tv','كاميرا','camera','سماعات','speaker','headphones','طابعة','printer'
    ],
  },
  {
    slug: 'motorcycles',
    name: 'دراجات نارية',
    keywords: ['دراجة','دراجات','دراجة نارية','دراجات نارية','موتور','موتوسيكل','motorcycle','motorcycles','bike','bikes'],
  },
  {
    slug: 'heavy_equipment',
    name: 'معدات ثقيلة',
    keywords: ['معدات','معدات ثقيلة','شيول','حفار','بوكلين','قلاب','جرافة','لودر','رافعة','crane','excavator','loader','heavy','equipment'],
  },
  {
    slug: 'solar',
    name: 'طاقة شمسية',
    keywords: ['طاقة شمسية','طاقة','الواح','ألواح','لوح','بطاريات','بطارية','انفرتر','انفرتير','inverter','battery','batteries','solar'],
  },
  {
    slug: 'networks',
    name: 'نت وشبكات',
    keywords: ['نت','شبكات','شبكه','شبكة','انترنت','إنترنت','واي فاي','wifi','راوتر','router','مودم','modem','سويتش','switch','ميكروتك','mikrotik','يوبيكويتي','ubiquiti'],
  },
  {
    slug: 'maintenance',
    name: 'صيانة',
    keywords: ['صيانة','تصليح','ورشة','ورشه','كهربائي','سباك','نجار','ميكانيكي','صيانة جوالات','صيانة سيارات','maintenance','repair'],
  },
  {
    slug: 'furniture',
    name: 'أثاث',
    keywords: ['اثاث','أثاث','كنب','مجلس','غرفة','غرف','سرير','دولاب','دواليب','طاولة','كرسي','furniture'],
  },
  {
    slug: 'home_tools',
    name: 'أدوات منزلية',
    keywords: ['ادوات منزلية','أدوات منزلية','مستلزمات منزلية','مطبخ','ثلاجة','غسالة','فرن','مكيف','home tools','appliances'],
  },
  {
    slug: 'clothes',
    name: 'ملابس',
    keywords: ['ملابس','ملابس نسائية','ملابس رجالية','أزياء','ازياء','clothes','fashion'],
  },
  {
    slug: 'animals',
    name: 'حيوانات وطيور',
    keywords: ['حيوانات','طيور','حيوان','غنم','ماعز','بقر','دجاج','حمام','قطط','كلاب','animal','animals','birds'],
  },
  {
    slug: 'jobs',
    name: 'وظائف',
    keywords: ['وظائف','وظيفة','توظيف','عمل','مطلوب موظف','job','jobs','hiring'],
  },
  {
    slug: 'services',
    name: 'خدمات',
    keywords: ['خدمات','خدمة','توصيل','نقل','شحن','تنظيف','تصميم','برمجة','service','services'],
  },
  {
    slug: 'other',
    name: 'أخرى',
    keywords: ['اخرى','أخرى','متفرقات','other'],
  },
];
...
function parsePriceFromText(messageRaw) {
  ...
  const unitRe = /(\d+(?:\.\d+)?)\s*(k|m|b|م|الف|الاف|آلاف|ألاف|مليون|ملايين|مليار|مليارات)\b/i;
  const m1 = unitRe.exec(raw);
  if (m1) {
    const n = Number(m1[1]);
    if (!isFinite(n) || n <= 0) return null;
    const u = normalizeText(m1[2]);
    let mult = 1;
    if (u === 'k') mult = 1000;
    else if (u === 'm' || u === 'م') mult = 1000000;
    else if (u === 'b') mult = 1000000000;
    else if (u.includes('الف') || u.includes('الاف') || u.includes('آلاف') || u.includes('ألاف')) mult = 1000;
    else if (u.includes('مليون') || u.includes('ملايين')) mult = 1000000;
    else if (u.includes('مليار') || u.includes('مليارات')) mult = 1000000000;
  ...
  if (!m2) return null;
  const numStr = m2[1].replace(/\s+/g, '');
  const n2 = Number(numStr);
...
function isBackCommand(messageRaw) {
  const t = normalizeText(String(messageRaw || '').trim().replace(/^\/+\s*/, ''));
  return t === 'رجوع' || t === 'عودة' || t === 'تراجع' || t === 'السابق' || t === 'back' || t.includes('ارجع');
}

function wizardCommandsHint() {
  return 'أوامر سريعة: عرض المسودة | رجوع | إلغاء | نشر';
}

function prevWizardStep(step) {
  const order = ['category', 'title', 'description', 'city', 'phone', 'location', 'price', 'confirm'];
  const idx = order.indexOf(String(step || 'category'));
...
function detectCurrency(messageRaw) {
  ...
  const t = normalizeText(raw);
  if (t.includes('sar') || t.includes('ريال سعودي') || t.includes('سعود') || t.includes('ر س') || t.includes('ر.س')) return 'SAR';
...
function listingNextPrompt(step, draft) {
  ...
  const head = summary ? `📌 المسودة حتى الآن:
${summary}
...
${wizardCommandsHint()}`;
  ...
  if (step === 'category') {
    return head + `الخطوة 1/7: اختر القسم (اكتب اسم القسم):
${categoriesHint()}

(تقدر تلغي بأي وقت بكتابة: إلغاء)` + foot;
  }

  if (step === 'title') {
    return head + `الخطوة 2/7: اكتب عنوان الإعلان.` + foot;
  }

  if (step === 'description') {
    return head + `الخطوة 3/7: اكتب وصف الإعلان (على الأقل 10 أحرف).
مثال: الحالة، الاستخدام، العيوب، سبب البيع.` + foot;
  }

  if (step === 'city') {
    return head + `الخطوة 4/7: اكتب اسم المدينة.` + foot;
  }

  if (step === 'phone') {
    return head + `الخطوة 5/7: اكتب رقم الجوال للتواصل (مثال: 777123456 أو +967777123456).` + foot;
  }
...
function extractLabeledFields(rawText) {
  ...
  function pickAfterLabel(labelRe) {
...
async function extractListingDetailsFromMessage(messageRaw, meta) {
  ...
  if (labeled) {
    ...
    if (labeled.title) out.title = labeled.title;
    if (labeled.description) out.description = labeled.description;
    if (labeled.city) out.city = labeled.city;
    if (labeled.phone) out.phone = labeled.phone;
    if (labeled.locationLabel) out.locationLabel = labeled.locationLabel;
    if (labeled.lat != null && labeled.lng != null) { out.lat = labeled.lat; out.lng = labeled.lng; }
    if (labeled.price != null) out.price = labeled.price;
