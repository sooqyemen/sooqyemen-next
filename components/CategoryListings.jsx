...
export default function CategoryListings({ category, initialListings = [] }) {
  ...
  // ✅ قراءة البحث من رابط الصفحة ?q=... (لتفعيل الوسوم/الهاشتاقات)
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const sp = new URLSearchParams(window.location.search);
    const v = (sp.get('q') || sp.get('search') || sp.get('query') || '').trim();
    if (!v) return;
    // URLSearchParams يفك الترميز تلقائيًا؛ نحول _ إلى مسافة
    setQ(v.replace(/_/g, ' '));
  }, [single]);
