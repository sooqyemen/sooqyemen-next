...
async function getServerIdToken() {
  ...
  // Note: Firebase Web API keys are meant to be public - they're safe to include in client-side code
  // The key is protected by Firebase Security Rules, not by keeping it secret
...
function getProjectId() {
  // Check environment variables first, then use default fallback
  const projectId = getEnv('FIREBASE_PROJECT_ID') || getEnv('NEXT_PUBLIC_FIREBASE_PROJECT_ID');
  
  // Fallback to the default project ID from firebaseClient.js
  // Note: This is safe as it's only used when env vars are not set (dev/test environments)
  // In production, always set FIREBASE_PROJECT_ID or NEXT_PUBLIC_FIREBASE_PROJECT_ID
  if (!projectId) {
...
async function firestoreRunQuery(structuredQuery, { revalidate = 60 } = {}) {
  ...
  if (!projectId) {
    logOnce('missing_project', '[firestoreRest] Missing FIREBASE_PROJECT_ID/NEXT_PUBLIC_FIREBASE_PROJECT_ID');
    return [];
  }
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    logOnce(`runquery_${res.status}`, `[firestoreRest] runQuery HTTP ${res.status} ${res.statusText}: ${text}`);
    return [];
  }
