...
function pickUserId(routeParams, propsParams) {
  // ✅ اضمن قراءة المعرف حتى لو تغيّر اسم الباراميتر
  const raw =
    routeParams?.id ??
    routeParams?.userId ??
    routeParams?.uid ??
...
export default function UserListingsPage({ params }) {
  ...
  const userId = useMemo(() => pickUserId(routeParams, params), [routeParams, params]);
  ...
  const fetchFirst = useCallback(async () => {
    ...
    // ✅ هنا كان سبب الخطأ: uid فاضي لأن params.id ما وصلت
    if (!uid) {
    ...
    try {
      ...
      // جرّب (where + orderBy) أولاً (أفضل ترتيب)
      ...
      // fallback: بدون orderBy (لو ما عندك index)
      if (!snap) {
        const s = await db.collection('listings').where('userId', '==', uid).limit(PAGE_SIZE).get();
        snap = s;
        pickedField = 'userId';
        orderedOk = false;
      }
      ...
      // ✅ لو fallback بدون ترتيب: أوقف pagination عشان loadMore ما ينهار
      setCanPaginate(orderedOk);
      setHasMore(orderedOk ? snap.docs.length === PAGE_SIZE : false);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const snap = await db
        .collection('listings')
        .where(ownerFieldUsed, '==', String(userId || '').trim())
        .orderBy('createdAt', 'desc')
  ...
  }, [canPaginate, hasMore, loadingMore, ownerFieldUsed, userId]);
  ...
  }, [hasMore, loading, loadingMore, loadMore, canPaginate]);
      ...
      <div className="container" style={{ paddingTop: 18, paddingBottom: 28 }}>
        ...
        {loading ? (
          ...
          <div className="card" style={{ padding: 18, border: '1px solid rgba(220,38,38,0.25)', background: '#fef2f2' }}>
            ...
            <button className="btn btnPrimary" style={{ marginTop: 10 }} onClick={fetchFirst}>
              إعادة المحاولة
            </button>
          </div>
        ) : filtered.length === 0 ? (
