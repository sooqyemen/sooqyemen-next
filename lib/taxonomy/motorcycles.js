...
export const MOTORCYCLE_MODELS_BY_BRAND = {
  ...
  // ✅ مهم: "غير مصنف/other" له موديلات عامة + سعات
  other: [
...
// ✅ FIX: تطبيع الماركة + رجوع موديلات حتى لـ other
export function getMotorcycleModelsByBrand(brandKey) {
  const raw = String(brandKey || '').trim();
  const normalized = normalizeMotorcycleBrand(raw) || raw;
  const safeKey = normalized || 'other';

  if (Array.isArray(MOTORCYCLE_MODELS_BY_BRAND?.[safeKey])) return MOTORCYCLE_MODELS_BY_BRAND[safeKey];
  return Array.isArray(MOTORCYCLE_MODELS_BY_BRAND?.other) ? MOTORCYCLE_MODELS_BY_BRAND.other : [];
}
...
export function inferMotorcycleFromListing(listing) {
  ...
  // ✅ حتى لو other: نقدر نلقط موديل عام
  const model =
