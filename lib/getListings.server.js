...
async function fetchListings(limit = 24) {
  ...
  try {
    ...
    // Try to filter by status='active' if the field exists
    // If it causes an error, we'll fall back to just ordering
    try {
      query = query.where('status', '==', 'active');
    } catch (error) {
      // Field might not exist, continue without filter
      console.log('[getListings] Status filter not available, querying all listings');
    }
    ...
    try {
      ...
      // Fallback to updatedAt or no ordering
      console.log('[getListings] createdAt ordering not available');
    }
